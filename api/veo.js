...
function buildPrompt(voiceStyle, compliment, elevenLabs) {
  const audioSection = elevenLabs
    ? (
      // ElevenLabs mode: Veo produces a SILENT clip whose only sound is the click
      // at the press. The voice is generated by ElevenLabs and composited in later,
      // anchored to the detected press moment — so Veo never touches the voice.
      `AUDIO — CRITICAL: The ONLY sound anywhere in this entire video is a single, short, sharp, distinct mechanical "CLICK" at the exact instant the paw presses the yellow button down. ` +
      `Everything else is pure digital silence: NO voice, NO speech, NO talking, NO words, NO singing, NO music, NO narration, and NO ambient room tone, breathing, rustling or background hiss of any kind. ` +
      `The clip must be completely silent until that one crisp click, and silent again immediately after it. Do NOT generate any spoken audio at all. ` +
      `\n\n` +
      `ACTION TIMING: Quickly — within the first 2 seconds — the cat reaches out and presses the yellow button down with one paw, producing the single CLICK. ` +
      `The cat's mouth stays shut the entire time; the cat never speaks or vocalises. ` +
      `\n\n` +
      `AFTER THE PRESS: the cat turns its head and stares directly into the camera — deeply grumpy, unblinking, utterly unbothered — in silence. ` +
      `\n\n`
    )
    : (
