...
function buildPrompt(voiceStyle, compliment, elevenLabs) {
  const audioSection = elevenLabs
    ? (
      // ElevenLabs mode: Veo's audio is completely discarded and replaced by
      // ffmpeg (real click + ElevenLabs VO). No audio instructions here —
      // describing sounds was triggering Veo's audio safety filter.
      `ACTION: The cat is already in frame. Within the first second it lifts one paw and presses the yellow button down. ` +
